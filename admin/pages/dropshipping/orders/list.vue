                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-2 col-md-2 col-sm-6 mb-2">
                                    <select v-model="selectedFilter" class="form-control" @change="filterData">
                                        <option value="">All Orders</option> <!-- Option for selecting all orders -->
                                        <option v-for="(status, index) in orderStatus" :key="index" :value="status.id">
                                            {{ status.name }}</option>
                                    </select>
                                        ...
                                        <table class="table w-100 table-wrapper">
                                            ...
                                            <tbody>
                                                <tr v-for="(item, index) in productdata" :key="index">
                                                    ...
                                                    <td>
                                                        <center>
                                                            <button type="button" @click="editdata(item.orderId)"><i
                                                                    class="fas fa-search-plus btnSize"></i></button>
                                                        </center>
                                                    </td>
...
const fetchData = async (page) => {
    try {
        loading.value = true;
        const response = await axios.get(`/order/allOrdersAdmin`, {
            params: {
                page: page,
                pageSize: pageSize,
        ...
        productdata.value = response.data.orderdata;
