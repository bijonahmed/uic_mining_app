...
class ProductController extends Controller
{
    ...
    public function categoryWiseProduct(Request $request)
    {
        ...
        if (!empty($pro_category)) {
            ...
            foreach ($selected_products as $pro_row) {
                $productsArr[] = [
    ...
    public function getProductList(Request $request)
    {
        ...
        $query = Product::orderBy('id', 'desc')
            ->join('produc_categories', 'product.id', '=', 'produc_categories.product_id')
            ->select('product.*', 'produc_categories.category_id', 'produc_categories.parent_id')
            ->orderBy('id', 'desc');
        ...
        $modifiedCollection = $paginator->getCollection()->map(function ($item) {
            ...
            $mainCategoryId = $item->parent_id; // The full string
            $firstCommaPos  = strpos($mainCategoryId, ','); // Find the position of the first comma
            $firstValue     = substr($mainCategoryId, 0, $firstCommaPos); // Extract the substring before the first comma
            //echo $firstValue; // Output the first value
            $subCategoryId  = $item->parent_id; // The full string
            $values         = explode(',', $subCategoryId); // Split the string into an array using ',' as the delimiter
            $lastValue      = end($values); // Get the last element of the array
            $mainCategory   = Categorys::where('id', $firstValue)->first();
            $subCategory    = Categorys::where('id', $lastValue)->first();

            return [
                'id'                => $item->id,
                'name'              => substr($item->name, 0, 250),
                'mainCategory'      => !empty($mainCategory->name) ? $mainCategory->name : "",
                'subCategory'       => !empty($subCategory->name) ? $subCategory->name : "",
